#!/usr/bin/env python3
"""
Custom API Call Testing Script with Correct Authentication
...
def test_custom_telemetry():
    """Test custom telemetry processing with authentication"""
    print("=== Custom Telemetry Processing (Authenticated) ===")
...
def test_batch_custom_telemetry():
    """Test batch processing with custom data and authentication"""
    print("=== Batch Custom Telemetry Processing (Authenticated) ===")
...
def test_anomaly_detection_custom():
    """Test anomaly detection with custom data and authentication"""
    print("=== Custom Anomaly Detection (Authenticated) ===")
    ...
    try:
        response = requests.post(f"{BASE_URL}/ml/anomalies", json=anomaly_test_data, headers=get_auth_headers())
        print(f"Status: {response.status_code}")
        print(f"Response: {json.dumps(response.json(), indent=2)}")
    except Exception as e:
        print(f"Error: {e}")
...
def main():
    """Run all custom API tests with proper authentication"""
    print("🚀 JPMorgan Financial APIs - Custom API Call Testing (Fixed Auth)")
    print("=" * 70)

    # Test basic endpoints (no auth required)
    test_health_check()

    # Test authenticated endpoints with correct token
    test_custom_telemetry()
    test_batch_custom_telemetry()
    test_anomaly_detection_custom()

    print("✅ Custom API testing completed with proper authentication!")
    print("\n🔑 Using test token: 'test_token' (hardcoded in TokenManager for testing)")
