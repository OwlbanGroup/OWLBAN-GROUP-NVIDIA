...
class TelemetryDatabase:
    """Optimized database handler for telemetry data with PostgreSQL and SQLite support"""
    ...
    def __init__(self, db_path: str = None):
        ...
        if self.is_postgres:
            ...
            try:
                ...
                telemetry_logger.get_logger().info("PostgreSQL connection pool initialized")
            except Exception as e:
                telemetry_logger.log_error(e, {'context': 'postgres_pool_init'})
                raise
        else:
            # SQLite setup
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            if self.is_postgres:
                ...
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_telemetry_operation ON telemetry_events(operation)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_telemetry_pfn ON telemetry_events(pfn)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_telemetry_device_class ON telemetry_events(device_class)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_telemetry_os_name ON telemetry_events(os_name)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_telemetry_composite ON telemetry_events(operation, timestamp DESC)')
            ...
            if self.is_postgres:
                ...
                ''')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_metrics_timestamp ON telemetry_metrics(timestamp DESC)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_metrics_name ON telemetry_metrics(metric_name)')
            else:
                cursor.execute('''
                    CREATE TABLE IF NOT EXISTS telemetry_metrics (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            telemetry_logger.get_logger().info(f"Database initialized successfully ({'PostgreSQL' if self.is_postgres else 'SQLite'})")
            ...
            with self.get_connection() as conn:
                ...
                cursor.execute('''
                    INSERT INTO telemetry_events (
                        timestamp, operation, pfn, version, event_name, shell_id,
                        event_flags, pg_name, dvc_sample, flags, edition, epoch, seq,
                        data_type, is_required, data_category, product, priv_tags, policies,
                        cv, boot_id, os_name, os_version, exp_id, app_id, app_version,
                        is_1p, as_id, local_id, device_class, dev_make, dev_model,
                        ticket_keys, user_local_id, tz, pn1, p1, pn2, p2, pn3, p3, pn4, p4
                ...
                ''', (
                    event.timestamp, event.operation, event.pfn, event.version, event.event_name,
                    event.shell_id, event.event_flags, event.pg_name, event.dvc_sample, event.flags,
                    event.edition, event.epoch, event.seq, event.data_type, event.is_required,
                    event.data_category, event.product, event.priv_tags, event.policies, event.cv,
                    event.boot_id, event.os_name, event.os_version, event.exp_id, event.app_id,
                    event.app_version, event.is_1p, event.as_id, event.local_id, event.device_class,
                    event.dev_make, event.dev_model, json.dumps(event.ticket_keys), event.user_local_id,
                    event.tz, event.pn1, event.p1, event.pn2, event.p2, event.pn3, event.p3, event.pn4, event.p4
                ))
                return True
        except Exception as e:
            telemetry_logger.log_error(e, {'context': 'storing_telemetry_event'})
            return False

    def store_events_batch(self, events: List[TelemetryEvent]) -> int:
        """Store multiple telemetry events in a single batch operation for better performance"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
    ...
    def get_events_by_operation(self, operation: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        return cache_database_query(expiration=300)(self._get_events_by_operation_uncached)(operation, limit)

    def _get_events_by_operation_uncached(self, operation: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                return {
                    ...
                    'avg_events_per_hour': round(avg_per_hour, 2) if avg_per_hour else 0
                }
        except Exception as e:
    ...
    def cleanup_old_events(self, days: int = 30) -> int:
        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                telemetry_logger.get_logger().info(f"Cleaned up {deleted_count} old events (older than {days} days)")
...
class TelemetryHandler:
    ...
    def process_batch(self, telemetry_data_list: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        try:
            ...
            async_results = asyncio.run(process_batch_async(telemetry_data_list))
            ...
            telemetry_logger.get_logger().error(f"Async batch processing failed: {e}")
