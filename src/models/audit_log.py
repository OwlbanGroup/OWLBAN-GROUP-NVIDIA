...
class AuditLogModel(Base):
    ...
    resource_type = Column(String(100), index=True)  # Type of resource accessed (e.g., 'user', 'business', 'asset')
    ...
    request_method = Column(String(10))  # HTTP method (GET, POST, PUT, DELETE)
    ...
    request_data = Column(Text)  # Request payload (sanitized, no sensitive data)
    response_data = Column(Text)  # Response payload (sanitized)
    error_message = Column(Text)  # Error message if operation failed
    
    # Security and compliance
    severity = Column(String(20), default='info', index=True)  # Severity: info, warning, error, critical
    category = Column(String(50), index=True)  # Category: authentication, authorization, data_access, etc.
    compliance_tags = Column(Text)  # JSON array of compliance tags (e.g., ['PCI-DSS', 'GDPR'])
    
    # Tamper-proof hash chain
    previous_hash = Column(String(64))  # SHA-256 hash of previous log entry
    current_hash = Column(String(64), nullable=False, index=True)  # SHA-256 hash of current entry
    ...
    @staticmethod
    def calculate_hash(log_data: Dict[str, Any], previous_hash: Optional[str] = None) -> str:
        """
    ...
    def verify_chain_integrity(logs: list):
        ...
        for i, log in enumerate(logs):
            # Verify current hash
            expected_hash = AuditLogModel.calculate_hash(log.to_dict(), previous_hash)
            if log.current_hash != expected_hash:
                return False, f"Hash mismatch at log ID {log.id}: expected {expected_hash}, got {log.current_hash}"
            
            # Verify chain link
            if i > 0 and log.previous_hash != logs[i-1].current_hash:
                return False, f"Chain broken at log ID {log.id}: previous_hash doesn't match"
...
class AuditLogSummary:
    ...
    def __init__(self, total_logs: int, by_action: Dict[str, int], by_severity: Dict[str, int],
                 by_user: Dict[str, int], failed_attempts: int, time_range: tuple):
