...
class DCGMMonitor:
    ...
    def _calculate_gpu_health(self, metrics: Dict[str, Any]) -> float:
        ...
        try:
            ...
            gpu_temp = metrics.get('gpu_temp_celsius', 0)
            if gpu_temp > 80:
        ...
        try:
            ...
            for gpu_key, gpu_data in gpu_stats.items():
                if gpu_key.startswith('gpu_'):
                    health_score = gpu_data.get('health_score', 0)
                    health_scores.append(health_score)

                    if health_score > 70:
                        system_stats['healthy_gpus'] += 1

                    system_stats['total_power_watts'] += gpu_data.get('power_usage_watts', 0)
                    system_stats['average_gpu_temp'] += gpu_data.get('gpu_temp_celsius', 0)
                    system_stats['average_gpu_utilization'] += gpu_data.get('gpu_utilization_percent', 0)
        ...
        try:
            ...
            for gpu_key, gpu_data in gpu_stats.items():
                if gpu_key.startswith('gpu_'):
                    ...
                    # Temperature alerts
                    gpu_temp = gpu_data.get('gpu_temp_celsius', 0)
                    if gpu_temp > 85:
                        alerts.append({
                            'gpu_id': gpu_id,
                            'severity': 'critical',
                    ...
                    # Health alerts
                    health_score = gpu_data.get('health_score', 100)
                    if health_score < 50:
                        alerts.append({
                            'gpu_id': gpu_id,
                            'severity': 'critical',
