...
try:
    import blpapi
    from blpapi import Name, Session, Service, Event, Message
...
class FinancialDataManager:
    ...
    def _initialize_connections(self):
        ...
        if BLOOMBERG_AVAILABLE:
            self.bloomberg_session = Session(blpapi.SessionOptions())
    ...
    async def get_historical_data(self,
        ...
        if provider == "bloomberg" and BLOOMBERG_AVAILABLE:
            return await self._get_bloomberg_historical(symbols, start_date, end_date, interval)
        elif provider == "refinitiv" and REFINITIV_AVAILABLE:
            return await self._get_refinitiv_historical(symbols, start_date, end_date, interval)
        else:
    ...
    async def get_fundamental_data(self,
        ...
        if provider == "bloomberg" and BLOOMBERG_AVAILABLE:
            return await self._get_bloomberg_fundamentals(symbols, metrics)
        elif provider == "refinitiv" and REFINITIV_AVAILABLE:
            return await self._get_refinitiv_fundamentals(symbols, metrics)
        else:
    ...
    async def get_market_analytics(self,
        ...
        if analysis_type in analytics_map:
            return await analytics_map[analysis_type](symbols, parameters)
        else:
