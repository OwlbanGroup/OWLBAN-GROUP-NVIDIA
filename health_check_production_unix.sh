...
echo "ðŸ” Starting production health checks for $RELEASE_NAME in namespace $NAMESPACE"
...
check_pods() {
    ...
    if [ "$pods_ready" -eq "$total_pods" ] && [ "$total_pods" -gt 0 ]; then
        echo "âœ… All $pods_ready pods are running"
        return 0
    else
        echo "âŒ Pod status check failed: $pods_ready/$total_pods pods running"
        return 1
    fi
}
export -f check_pods
...
check_services() {
    ...
    if [ -n "$service_ip" ]; then
        echo "âœ… Service is available at $service_ip"
        return 0
    else
...
check_application_health() {
    ...
    if [ -n "$pod_name" ]; then
        ...
        if [ "$health_status" != "failed" ]; then
            ...
            echo "âŒ Application health check failed"
            return 1
        fi
    else
        echo "âŒ No application pods found"
        return 1
    fi
}
...
" 2>/dev/null)
        ...
        if [ "$db_status" = "connected" ]; then
            ...
            echo "âŒ Database connectivity check failed"
            return 1
        fi
    else
        echo "âŒ No application pods found for database check"
        return 1
    fi
}
...
" 2>/dev/null)
        ...
        if [ "$redis_status" = "connected" ]; then
            ...
            echo "âŒ Redis connectivity check failed"
            return 1
        fi
    else
        echo "âŒ No application pods found for Redis check"
        return 1
    fi
}
...
check_monitoring() {
    ...
    if [ "$prom_pods" -gt 0 ]; then
        ...
        echo "âŒ Prometheus check failed"
        return 1
    fi
    ...
    if [ "$es_pods" -gt 0 ]; then
        echo "âœ… Elasticsearch is running ($es_pods pods)"
    else
        echo "âŒ Elasticsearch check failed"
        return 1
    fi
...
check_istio() {
    echo "ðŸ” Checking Istio configuration..."
    ...
    if [ "$istio_enabled" -gt 0 ]; then
        echo "âœ… Istio PeerAuthentication is configured"
    else
        echo "âš ï¸ Istio PeerAuthentication not found"
    fi

    local gateway_count=$(kubectl get gateway -n $NAMESPACE -o name | wc -l)
    if [ "$gateway_count" -gt 0 ]; then
        echo "âœ… Istio Gateway is configured"
    else
        echo "âŒ Istio Gateway not found"
        return 1
    fi

    local vs_count=$(kubectl get virtualservice -n $NAMESPACE -o name | wc -l)
    if [ "$vs_count" -gt 0 ]; then
        echo "âœ… Istio VirtualService is configured"
    else
        echo "âŒ Istio VirtualService not found"
        return 1
    fi
...
run_load_test_validation() {
    echo "ðŸš€ Running load test validation..."
    ...
    if [ -d "load-testing" ]; then
        ...
        if npm run test:smoke; then
            echo "âœ… Load test smoke validation passed"
            return 0
        else
            echo "âŒ Load test smoke validation failed"
            return 1
        fi
    else
        echo "âš ï¸ Load testing directory not found, skipping validation"
        return 0
    fi
}
...
# Main execution
echo "â° Starting health checks with $TIMEOUT second timeout..."
...
if [ $? -ne 0 ]; then
    echo "âŒ Health checks timed out or failed"
    exit 1
fi
